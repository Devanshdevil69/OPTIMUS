...
@amaan.on_message(filters.command("purge", SUDO_HANDLER) & filters(SUDO_USERS))
async def purge(client, message):
    if message.reply_to_message:
        datetime.now()
        from_user = None
...
@amaan.on_message(filters.command("purgeme", SUDO_HANDLER) & filters(SUDO_USERS))
async def purge_myself(client, message):
    if len(message.text.split()) >= 2 and message.text.split()[1].isdigit():
        target = int(message.text.split()[1])
    else:
...
@amaan.on_message(filters.command("del", SUDO_HANDLER) & filters(SUDO_USERS))
async def delete_replied(client, message):
    msg_ids = [message.message_id]
    if message.reply_to_message:
        msg_ids.append(message.reply_to_message.message_id)
